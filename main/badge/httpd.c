...
static esp_err_t tetris_result_handler(httpd_req_t *req, const char *client_data)
{
    ...
    cJSON *player = cJSON_GetObjectItem(client_json, "player_b64");
    cJSON *points = cJSON_GetObjectItem(client_json, "points");

    esp_err_t err;
    if (cJSON_IsString(player) && (player->valuestring != NULL) && cJSON_IsNumber(points))
    {

        size_t out_len = 0;
        mbedtls_base64_decode(NULL, 0, &out_len, (const unsigned char *)player->valuestring, strlen(player->valuestring));
        ...
        if (mbedtls_base64_decode(decoded, out_len, &out_len,
                                  (const unsigned char *)player->valuestring, strlen(player->valuestring)) == 0)
        {
            char buf[24];
            memcpy(buf, decoded, out_len);

            printf("Buf after memcpy:\n");
            for (size_t i = 0; i < (out_len < 48 ? out_len : 48); i++)
            {
                printf("%02X ", buf[i]);
            }
            printf("\n");
    ...
    {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed on tetris_result_handler() function");
        err = ESP_FAIL;
    }
...
static esp_err_t post_handler(httpd_req_t *req)
{
    ...
    else if (is_string_match(cmd, "tetrisresult"))
    {
        tetris_result_handler(req, buf);
    }
    else
    {
