...
async function query_wifi_client(key, ssid, password) {
  return query('wifi_client', { key: key, wifi_client: { ssid: ssid, password: password } }).then((data) => {
    return data.wifi_client;
  }).catch(error => {
    return Promise.reject(error);//console.error('There has been a problem with your fetch operation:', error);
  });
  //return query('wifi', {key: key, wifi:{ssid: ssid, password: password, status: status}});
...
async function query_ntp_server(key, ntp_server) {
  return query('ntp_server', { key: key, ntp_server: ntp_server }).then((data) => {
    return data.ntp_server;
  }).catch(error => {
