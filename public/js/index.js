...
var wifi_client_ssid_form = document.querySelector("#wifi-client-ssid-form");
var wifi_client_ssid_input = document.querySelector("#wifi-client-ssid");
...
var wifi_client_password_form = document.querySelector("#wifi-client-password-form");
...
var wifi_client_password_input1 = document.querySelector("#wifi-client-password1");
var wifi_client_password_input2 = document.querySelector("#wifi-client-password2");
...
async function check_authentication(key) {
  query_authentication(key)
    .then(() => {
      ...
      query_wifi_client(key, null).then(wifi_client => {
        setInput(wifi_client_ssid_input, wifi_client.ssid);
      });
      query_ntp_server(key, null).then(ntp => {
        setInput(ntp_input, ntp);
      });
    })
...
ntp_form.addEventListener('submit', function (evt) {
  ...
  if ((ntp_input.length > 0)) {
    query_ntp_server(getCookie('key'), ntp_input.value).then(() => Toastify({
      ...
      .catch(() => Toastify({
        text: "Error. Cannot change NTP server.",
        duration: 3000,
        position: "center",
        style: {
          background: "#dc3545",
...
wifi_client_password_form.addEventListener('submit', function (evt) {
  evt.preventDefault();
  if ((wifi_client_password_input1.value.length > 0) && (wifi_client_password_input2.value.length > 0)) {
    if (wifi_client_password_input1.value === wifi_client_password_input2.value) {
      query_wifi_client(getCookie("key"), null, wifi_client_password_input1.value, null)
        .then(() => {
          resetInput(wifi_client_password_input1);
          resetInput(wifi_client_password_input2);
        })
        .then(() => Toastify({
          text: "WiFi password successfully changed.",
          duration: 3000,
          position: "center",
          style: {
            background: "#28a745",
