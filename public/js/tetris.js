...
const sendScoreDiv = document.getElementById('scoreform');
const btnSendScore = document.getElementById('btnSendScore');
const sendScorePlayerName = document.getElementById('sendScorePlayerName');
...
const checkGameOver = () => {
    if ((currentPosition < width * 2 &&
        ...
        sendScoreDiv.style.display = "block";
...
const buttonPlay = () => {
    ...
    sendScoreDiv.style.display = "none";
    if (runGame && !gameOver) {
        colorTetromino(randomIndex1);
        colorUpNextTetromino(randomIndex2);
...
const sendScore = async function () {
    let playerName = sendScorePlayerName.value;
    let playerScore = currentScore;
    if (!playerName) playerName = "Anonymous";
    if (playerName.length > 24) {
        Toastify({
            text: "Error. Name too long.",
            duration: 3000,
            position: "center",
            style: {
                background: "#dc3545",
    ...
    let playerName_b64 = btoa(playerName);

    return query("tetrisresult", { player_b64: playerName_b64, points: playerScore }).then(
        () => {
            Toastify({
                text: "Submitted.",
                duration: 3000,
...
btnSendScore.addEventListener('click', sendScore);
